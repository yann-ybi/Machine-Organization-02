...
impl<T: Clone> Array2<T>{
    ...
    pub fn iter_row_major(&mut self) -> impl Iterator <Item = (usize, usize, usize)> {

        let _len = self.width_height.0 * self.width_height.1;
        let mut xyi_path: Vec<(usize, usize, usize)> = Vec::new();

        for _i in 0.._len{
            let index = self.width_height.0 * self.col_row.1 + self.col_row.0;
            xyi_path.push((self.col_row.0, self.col_row.1, index));
            
            self.next();
        }
        return xyi_path.into_iter();
    }
    pub fn iter_col_major(&mut self)-> impl Iterator <Item = (usize, usize, usize)>{ 
        ...
        for _z in 0..width {
            ...
            for _i in 0..height{ 
                let index = width * self.col_row.1 + self.col_row.0;
...
/*  use super::*;
    ...
    fn test_iter_col_major() {
        let random = Array2::blanck_state(0, (9,9));
